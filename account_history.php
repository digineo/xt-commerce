<?php
/* -----------------------------------------------------------------------------------------
   $Id: account_history.php,v 1.6 2004/06/03 20:09:19 fanta2k Exp $   
  ...
  if (($orders_total = xtc_count_customer_orders()) > 0) {
    ...
    while ($history = xtc_db_fetch_array($history_query)) {
      ...
      $module_content[]=array(
                            ...
                            'ORDER_BUTTON'=>'<a href="' . xtc_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'page=' . $_GET['page'] . '&order_id=' . $history['orders_id'], 'SSL') . '">' . xtc_image_button('small_view.gif', SMALL_IMAGE_BUTTON_VIEW) . '</a>');
