<?php
/* --------------------------------------------------------------
   $Id: products_options.php 243 2007-03-08 13:35:19Z mzanier $   
  ...
  $type_array = array();
  $type_array[]=array('id'=>'0','text'=>'-- Please Select --');
  $type_array[]=array('id'=>'1','text'=>'Dropdown');
  $type_array[]=array('id'=>'2','text'=>'Text');
  $type_array[]=array('id'=>'3','text'=>'Textarea');
  $type_array[]=array('id'=>'4','text'=>'Radio');
  $type_array[]=array('id'=>'6','text'=>'Checkbox');
  $type_array[]=array('id'=>'7','text'=>'Read Only');
    ...
    switch($_GET['action']) {
      case 'add_product_options':
        for ($i = 0, $n = sizeof($languages); $i < $n; $i ++) {
          ...
          xtc_db_query("insert into " . TABLE_PRODUCTS_OPTIONS . " (products_options_id,products_options_name, language_id,products_options_type,products_options_length,products_options_rows,products_options_size) values ('" . $_POST['products_options_id'] . "', '" . $option_name[$languages[$i]['id']] . "', '" . $languages[$i]['id'] . "','".$option_type."','".$option_length."','".$option_rows."','".$option_size."')");
        }
        xtc_redirect(xtc_href_link(FILENAME_PRODUCTS_OPTIONS, $page_info)); 
        break;
      ...
      case 'update_option_name':
        for ($i = 0, $n = sizeof($languages); $i < $n; $i ++) {
          ...
          $option_length = (int)$_POST['option_length'];
          $option_type = (int)$_POST['options_type']; 
          xtc_db_query("update " . TABLE_PRODUCTS_OPTIONS . " set products_options_name = '" . $option_name[$languages[$i]['id']] . "' where products_options_id = '" . $id . "' and language_id = '" . $languages[$i]['id'] . "'");
          // update fields
          xtc_db_query("update " . TABLE_PRODUCTS_OPTIONS . " set products_options_type = '" . $option_type . "' where products_options_id = '" . $id . "' and language_id = '" . $languages[$i]['id'] . "'");	
          xtc_db_query("update " . TABLE_PRODUCTS_OPTIONS . " set products_options_length = '" . $option_length . "' where products_options_id = '" . $id . "' and language_id = '" . $languages[$i]['id'] . "'");	
          xtc_db_query("update " . TABLE_PRODUCTS_OPTIONS . " set products_options_rows = '" . $option_rows . "' where products_options_id = '" . $id . "' and language_id = '" . $languages[$i]['id'] . "'");	
          xtc_db_query("update " . TABLE_PRODUCTS_OPTIONS . " set products_options_size = '" . $option_size . "' where products_options_id = '" . $id . "' and language_id = '" . $languages[$i]['id'] . "'");	
       ...
       for ($i = 0, $n = sizeof($languages); $i < $n; $i ++) {
		...
		for ($img = 0; $img < MO_PICS; $img ++) {
			if ($pIMG = & xtc_try_upload('mo_pics_'.$img, DIR_FS_CATALOG_IMAGES.'product_options/', '777', '')) {
				...
				$mo_img = array ('products_options_values_id' => xtc_db_prepare_input($_POST['value_id']), 'image_nr' => xtc_db_prepare_input($img +1), 'image_name' => xtc_db_prepare_input($value_image_name));
//				if ($action == 'insert') {
...
function go_option() {
  if (document.option_order_by.selected.options[document.option_order_by.selected.selectedIndex].value != "none") {
    location = "<?php echo xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'option_page=' . ($_GET['option_page'] ? $_GET['option_page'] : 1)); ?>&option_order_by="+document.option_order_by.selected.options[document.option_order_by.selected.selectedIndex].value;
  }
}
//--></script>
</head>
...
?>
                  ...
                  <tr>
                    <td align="right" colspan="3" class="main"><br /><?php echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_OPTIONS, '&value_page=' . $_GET['value_page'] . '&attribute_page=' . $attribute_page, 'NONSSL'));?>&nbsp;</td>
                  </tr>
<?php
    } else {
?>
                  ...
                  <tr>
                    <td class="main" align="right" colspan="3"><br /><?php echo xtc_button_link(BUTTON_DELETE, xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'action=delete_option&option_id=' . $_GET['option_id'], 'NONSSL'));?>&nbsp;&nbsp;&nbsp;<?php echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_OPTIONS, '&order_by=' . $order_by . '&page=' . $page, 'NONSSL'));?>&nbsp;</td>
                  </tr>
<?php
    }
?>
...
?>
  ...
  <tr>
    ...
    <td class="pageHeading"><?php echo HEADING_TITLE_OPT; ?></td>
  </tr>
  <tr>
...
<?php
    ...
    // Previous
    if ($prev_option_page)  {
      echo '<a href="' . xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'option_page=' . $prev_option_page.'&searchoption='.$_GET['searchoption']) . '"> &lt;&lt; </a> | ';
    }

    for ($i = 1; $i <= $num_pages; $i++) {
      if ($i != $option_page) {
        echo '<a href="' . xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'option_page=' . $i.'&searchoption='.$_GET['searchoption']) . '">' . $i . '</a> | ';
      } else {
    ...
    if ($option_page != $num_pages) {
      echo '<a href="' . xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'option_page=' . $next_option_page.'&searchoption='.$_GET['searchoption']) . '"> &gt;&gt; </a>';
    }
?>
                </td>
              </tr>
...
<?php
      if (($_GET['action'] == 'update_option') && ($_GET['option_id'] == $options_values['products_options_id'])) {
        echo '<form name="option" action="' . xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'action=update_option_name&option_page='.$_GET['option_page'], 'NONSSL') . '" method="post">';
        $inputs = '';
        for ($i = 0, $n = sizeof($languages); $i < $n; $i ++) {
          $option_name = xtc_db_query("select * from " . TABLE_PRODUCTS_OPTIONS . " where products_options_id = '" . $options_values['products_options_id'] . "' and language_id = '" . $languages[$i]['id'] . "'");
          $option_name = xtc_db_fetch_array($option_name);
          $type = $option_name['products_options_type'];
          $inputs .= xtc_image(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/admin/images/'.$languages[$i]['image']) . '&nbsp;<input type="text" name="option_name[' . $languages[$i]['id'] . ']" size="20" value="' . $option_name['products_options_name'] . '">&nbsp;<br />';
        }
?>
                <td align="center" class="smallText">&nbsp;<?php echo $options_values['products_options_id']; ?><input type="hidden" name="option_id" value="<?php echo $options_values['products_options_id']; ?>">&nbsp;</td>
                <td class="smallText"><?php echo $inputs; ?></td>
                 <td class="smallText"><?php echo xtc_draw_pull_down_menu('options_type',$type_array,$type_array[$type]['id']); ?>
                 </td>
                 <td class="smallText">
                ...
                <td align="center" class="smallText">&nbsp;<?php echo xtc_button(BUTTON_UPDATE); ?>&nbsp;<?php echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_OPTIONS, '', 'NONSSL'));?>&nbsp;</td>
<?php
        echo '</form>' . "\n";
      } else {
?>
                ...
                <td align="center" class="smallText">&nbsp;<?php echo xtc_button_link(BUTTON_EDIT, xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'action=update_option&option_id=' . $options_values['products_options_id'] . '&option_order_by=' . $option_order_by . '&option_page=' . $option_page, 'NONSSL'));?>&nbsp;&nbsp;<?php echo xtc_button_link(BUTTON_DELETE, xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'action=delete_product_option&option_id=' . $options_values['products_options_id'], 'NONSSL'));?>&nbsp;</td>
<?php
      }
?>
              </tr>
...
echo '<form name="options" action="' . xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'action=add_product_options&option_page=' . $option_page, 'NONSSL') . '" method="post"><input type="hidden" name="products_options_id" value="' . $next_id . '">';
?> 
              <tr class="dataTableRowSelected">
<?php
       $inputs = '';
      for ($i = 0, $n = sizeof($languages); $i < $n; $i ++) {
        $inputs .= xtc_image(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/admin/images/'.$languages[$i]['image']). '&nbsp;<input type="text" name="option_name[' . $languages[$i]['id'] . ']" size="20">&nbsp;<br />';
      }
?>
                <td align="center" class="dataTableContent">&nbsp;<?php echo $next_id; ?>&nbsp;</td>
                <td class="dataTableContent"><?php echo $inputs; ?></td>
                <td class="dataTableContent"><?php echo xtc_draw_pull_down_menu('options_type',$type_array); ?></td>
                <td align="center" class="dataTableContent" colspan="4">&nbsp;<?php echo xtc_button(BUTTON_INSERT); ?>&nbsp;</td>
                </tr>   
              <tr class="dataTableRowSelected">
                <td align="center" class="dataTableContent">&nbsp;</td>
                <td class="dataTableContent" colspan="6">
                <?php echo TEXT_NOTE; ?><br />
...
?>
                  ...
                  <tr>
                    <td class="main" align="right" colspan="3"><br /><?php echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_OPTIONS, '&value_page=' . $_GET['value_page'] . '&attribute_page=' . $attribute_page, 'NONSSL'));?>&nbsp;</td>
                  </tr>
<?php
    } else {
?>
                  ...
                  <tr>
                    <td class="main" align="right" colspan="3"><br /><?php echo xtc_button_link(BUTTON_DELETE, xtc_href_link(FILENAME_PRODUCTS_OPTIONS, 'action=delete_value&value_id=' . $_GET['value_id'], 'NONSSL')); ?>&nbsp;&nbsp;&nbsp;<?php echo xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_PRODUCTS_OPTIONS, '&option_page=' . $option_page . '&value_page=' . $_GET['value_page'] . '&attribute_page=' . $attribute_page, 'NONSSL'));?>&nbsp;</td>
                  </tr>
<?php
    }
?>
