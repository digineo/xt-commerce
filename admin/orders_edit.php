...
if ($_GET['action'] == "product_option_edit") {
	...
	$sql_data_array = array (
		'products_options' => xtc_db_prepare_input($_POST['products_options']
	), 'products_options_values' => xtc_db_prepare_input($_POST['products_options_values']), 'options_values_price' => xtc_db_prepare_input($_POST['options_values_price']));

	$update_sql_data = array (
		'price_prefix' => xtc_db_prepare_input($_POST['prefix']
	));
	$sql_data_array = xtc_array_merge($sql_data_array, $update_sql_data);
...
if ($_GET['action'] == "product_option_ins") {
	...
	$sql_data_array = array (
		'orders_id' => xtc_db_prepare_input($_POST['oID']
	), 'orders_products_id' => xtc_db_prepare_input($_POST['opID']), 'products_options' => xtc_db_prepare_input($products_options['products_options_name']), 'products_options_values' => xtc_db_prepare_input($products_options_values['products_options_values_name']), 'options_values_price' => xtc_db_prepare_input($products_attributes['options_values_price']));
	...
	if (DOWNLOAD_ENABLED == 'true') {
		...
		if (isset ($attributes_values['products_attributes_filename']) && xtc_not_null($attributes_values['products_attributes_filename'])) {
			$sql_data_array = array (
				'orders_id' => xtc_db_prepare_input($_POST['oID']
			), 'orders_products_id' => xtc_db_prepare_input($_POST['opID']), 'orders_products_filename' => $attributes_values['products_attributes_filename'], 'download_maxdays' => $attributes_values['products_attributes_maxdays'], 'download_count' => $attributes_values['products_attributes_maxcount']);
...
if ($_GET['action'] == "shipping_edit") {
	...
	if (file_exists(DIR_FS_LANGUAGES . $order->info['language'] . '/modules/shipping/' . $module)) {
		...
		$sql_data_array = array (
			'orders_id' => xtc_db_prepare_input($_POST['oID']
		), 'title' => xtc_db_prepare_input($shipping_text), 'text' => xtc_db_prepare_input($text), 'value' => xtc_db_prepare_input($_POST['value']), 'class' => 'ot_shipping');
...
if ($_GET['action'] == "ot_edit") {
	...
	if (xtc_db_num_rows($check_total_query)) {
		...
		), 'text' => xtc_db_prepare_input($text), 'value' => xtc_db_prepare_input($_POST['value']),);
		...
		$sql_data_array = array (
			'orders_id' => xtc_db_prepare_input($_POST['oID']
		), 'title' => xtc_db_prepare_input($_POST['title']), 'text' => xtc_db_prepare_input($text), 'value' => xtc_db_prepare_input($_POST['value']), 'class' => xtc_db_prepare_input($_POST['class']), 'sort_order' => xtc_db_prepare_input($_POST['sort_order']),);
...
if ($_GET['action'] == "save_order") {
	...
	while ($products = xtc_db_fetch_array($products_query)) {
		...
		$sql_data_array = array (
			'orders_id' => xtc_db_prepare_input($_POST['oID']
		), 'n_price' => xtc_db_prepare_input($nprice), 'b_price' => xtc_db_prepare_input($bprice), 'tax' => xtc_db_prepare_input($tax), 'tax_rate' => xtc_db_prepare_input($products['products_tax']));
	...
	while ($module_value = xtc_db_fetch_array($module_query)) {
		...
		$sql_data_array = array (
			'orders_id' => xtc_db_prepare_input($_POST['oID']
		), 'n_price' => xtc_db_prepare_input($module_n_price), 'b_price' => xtc_db_prepare_input($module_b_price), 'tax' => xtc_db_prepare_input($module_tax), 'tax_rate' => xtc_db_prepare_input($module_tax_rate));
	...
	while ($ust = xtc_db_fetch_array($ust_query)) {
		...
		if ($ust['tax_value_new']) {
			...
			$sql_data_array = array (
				'orders_id' => xtc_db_prepare_input($_POST['oID']
			), 'title' => xtc_db_prepare_input($title), 'text' => xtc_db_prepare_input($text), 'value' => xtc_db_prepare_input($ust['tax_value_new']), 'class' => 'ot_tax');
...
switch ($_GET['action']) {
	...
	default :
		if (is_object($order)) {
			$heading[] = array (
				'text' => '<b>' . TABLE_HEADING_ORDER . $_GET['oID'] . '</b>'
			);

			$contents[] = array (
				'align' => 'center',
				'text' => '<br />' . TEXT_EDIT_ADDRESS . '<br /><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_ORDERS_EDIT,
				'edit_action=address&oID=' . $_GET['oID']
			) . '">' . BUTTON_EDIT . '</a><br /><br />');
			$contents[] = array (
				'align' => 'center',
				'text' => '<br />' . TEXT_EDIT_PRODUCTS . '<br /><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_ORDERS_EDIT,
				'edit_action=products&oID=' . $_GET['oID']
			) . '">' . BUTTON_EDIT . '</a><br /><br />');
			$contents[] = array (
				'align' => 'center',
				'text' => '<br />' . TEXT_EDIT_OTHER . '<br /><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_ORDERS_EDIT,
				'edit_action=other&oID=' . $_GET['oID']
			) . '">' . BUTTON_EDIT . '</a><br /><br />');
