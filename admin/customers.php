...
/* --------------------------------------------------------------
   $Id: customers.php 292 2007-03-26 10:07:49Z mzanier $
...
if ($_GET['action']) {
	switch ($_GET['action']) {
		case 'new_order' :
			...
			// shipping
//			$insert_sql_data = array(
//				'shipping_method' => 'Pauschale Versandkosten', 'shipping_class' => 'flat_flat');
//			$sql_data_array = xtc_array_merge($sql_data_array, $insert_sql_data);	
			...
			include ('../lang/' . $lang_data['directory'] . '/modules/order_total/ot_total.php');
			include ('../lang/' . $lang_data['directory'] . '/modules/order_total/ot_subtotal.php');
		...
		case 'update' :
			...
			if ($memo_text != '' && $memo_title != '') {
				...
				), 'memo_title' => $memo_title, 'memo_text' => $memo_text, 'poster_id' => $_SESSION['customer_id']);
			...
			if ($error == false) {
				...
				xtc_redirect(xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array (
					'cID',
					'action'
				)) . 'cID=' . $customers_id));
			}
...
?>
	...
	echo $customers_statuses_array[$customers['customers_status']]['text'];
?></td>
...
?>
<div class="navigationSelection"><ul class="navigationList">
<li>
...
<?php
	...
	echo xtcIndexNavigation(FILENAME_CUSTOMERS);
?>
</ul></div>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent"><?php echo xtc_sorting(FILENAME_CUSTOMERS,'customers_lastname'); ?></td>
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_FIRSTNAME; ?></td>
                <td class="dataTableHeadingContent"><?php echo xtc_sorting(FILENAME_CUSTOMERS,'customers_firstname'); ?></td>
                ...
                <td class="dataTableHeadingContent" align="right"><?php echo xtc_sorting(FILENAME_CUSTOMERS,'date_account_created'); ?></td>
...
<?php
	...
	if (($_GET['search']) && (xtc_not_null($_GET['search']))) {
		...
		$search = "and (c.customers_lastname like '%" . $keywords . "%' or c.customers_telephone like '%" . $keywords . "%' or c.customers_vat_id like '%" . $keywords . "%' or c.customers_firstname like '%" . $keywords . "%' or c.customers_email_address like '%" . $keywords . "%')";
	}
	...
	while ($customers = xtc_db_fetch_array($customers_query)) {
		...
		if ((is_object($cInfo)) && ($customers['customers_id'] == $cInfo->customers_id)) {
			echo '          <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array (
				'cID',
				'action'
			)) . 'cID=' . $cInfo->customers_id . '&action=edit') . '\'">' . "\n";
		} else {
			echo '          <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array (
				'cID'
...
?></td>
                <td class="dataTableContent" colspan="2"><b><?php echo $customers['customers_lastname']; ?></b></td>
                <td class="dataTableContent" colspan="2"><?php echo $customers['customers_firstname']; ?></td>
...
?>
                ...
                <?php } ?>
                <td class="dataTableContent" colspan="2" align="right"><?php echo xtc_date_short($info['date_account_created']); ?></td>
              ...
              <tr>
                <td colspan="4"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td class="smallText" align="right"><?php echo $customers_split->display_links($customers_query_numrows, '20', MAX_DISPLAY_PAGE_LINKS, $_GET['page'], xtc_get_all_get_params(array('page', 'info','action', 'x', 'y', 'cID'))); ?></td>
                  </tr>
<?php
...
?>
<tr>
                    <td align="right" colspan="2"><?php echo '<a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_CREATE_ACCOUNT) . '">' . BUTTON_CREATE_ACCOUNT . '</a>'; ?>
</td>
                  </tr>
                </table></td>
              </tr>
...
<?php
	...
	switch ($_GET['action']) {
		case 'confirm' :
			...
			$contents = array (
				'form' => xtc_draw_form('customers',
				FILENAME_CUSTOMERS,
				xtc_get_all_get_params(array (
					'cID',
					'action'
				)
			) . 'cID=' . $cInfo->customers_id . '&action=deleteconfirm'));
			$contents[] = array (
				'text' => TEXT_DELETE_INTRO . '<br /><br /><b>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</b>'
			);
			if ($cInfo->number_of_reviews > 0)
				$contents[] = array (
					'text' => '<br />' . xtc_draw_checkbox_field('delete_reviews',
					'on',
					true
				) . ' ' . sprintf(TEXT_DELETE_REVIEWS, $cInfo->number_of_reviews));
			$contents[] = array (
				'align' => 'center',
				'text' => '<br /><input type="submit" class="button" value="' . BUTTON_DELETE . '"><a class="button" href="' . xtc_href_link(FILENAME_CUSTOMERS,
				xtc_get_all_get_params(array (
					'cID',
					'action'
				)
			) . 'cID=' . $cInfo->customers_id) . '">' . BUTTON_CANCEL . '</a>');
			break;
		...
		case 'editstatus' :
			if ($_GET['cID'] != 1) {
				...
				$heading[] = array (
					'text' => '<b>' . TEXT_INFO_HEADING_STATUS_CUSTOMER . '</b>'
				);
				$contents = array (
					'form' => xtc_draw_form('customers',
					FILENAME_CUSTOMERS,
				...
				) . 'cID=' . $cInfo->customers_id . '&action=statusconfirm'));
				$contents[] = array (
					'text' => '<br />' . xtc_draw_pull_down_menu('status',
					$customers_statuses_array,
					$cInfo->customers_status
				));
				$contents[] = array (
					'text' => '<table nowrap border="0" cellspacing="0" cellpadding="0"><tr><td style="border-bottom: 1px solid; border-color: #000000;" nowrap class="smallText" align="center"><b>' . TABLE_HEADING_NEW_VALUE . ' </b></td><td style="border-bottom: 1px solid; border-color: #000000;" nowrap class="smallText" align="center"><b>' . TABLE_HEADING_DATE_ADDED . '</b></td></tr>'
				);
				...
				if (xtc_db_num_rows($customers_history_query)) {
					while ($customers_history = xtc_db_fetch_array($customers_history_query)) {
						...
						$contents[] = array (
							'text' => '<tr>' . "\n" . '<td class="smallText">' . $customers_statuses_array[$customers_history['new_value']]['text'] . '</td>' . "\n" . '<td class="smallText" align="center">' . xtc_datetime_short($customers_history['date_added']
				...
				$contents[] = array (
					'align' => 'center',
					'text' => '<br /><input type="submit" class="button" value="' . BUTTON_UPDATE . '"><a class="button" href="' . xtc_href_link(FILENAME_CUSTOMERS,
					xtc_get_all_get_params(array (
						'cID',
						'action'
					)
		...
		default :
			...
			if (is_object($cInfo)) {
				$heading[] = array (
					'text' => '<b>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</b>'
				);
				if ($cInfo->customers_id != 1) {
					$contents[] = array (
						'align' => 'center',
						'text' => '<a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_CUSTOMERS,
						xtc_get_all_get_params(array (
							'cID',
							'action'
						)
					) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . BUTTON_EDIT . '</a>');
				}
				...
				if ($cInfo->customers_id != 1) {
					...
					) . 'cID=' . $cInfo->customers_id . '&action=confirm') . '">' . BUTTON_DELETE . '</a>');
				}
				if ($cInfo->customers_id != 1 /*&& $_SESSION['customer_id'] == 1*/
					) {
					$contents[] = array (
					...
					) . 'cID=' . $cInfo->customers_id . '&action=editstatus') . '">' . BUTTON_STATUS . '</a>');
				}
				// elari cs v3.x changed for added accounting module
				if ($cInfo->customers_id != 1) {
					$contents[] = array (
						'align' => 'center',
						'text' => '<a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_ACCOUNTING,
						xtc_get_all_get_params(array (
							'cID',
							'action'
						)
					) . 'cID=' . $cInfo->customers_id) . '">' . BUTTON_ACCOUNTING . '</a>');
				}
				// elari cs v3.x changed for added iplog module
				$contents[] = array (
					'align' => 'center',
					'text' => '<table><tr><td style="text-align: center;"><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_ORDERS,
					'cID=' . $cInfo->customers_id
				) . '">' . BUTTON_ORDERS . '</a></td><td style="text-align: center;"><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $cInfo->customers_email_address) . '">' . BUTTON_EMAIL . '</a></td></tr><tr><td style="text-align: center;"><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array (
					'cID',
					'action'
				)) . 'cID=' . $cInfo->customers_id . '&action=iplog') . '">' . BUTTON_IPLOG . '</a></td><td style="text-align: center;"><a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array (
					'cID',
					'action'
				)) . 'cID=' . $cInfo->customers_id . '&action=new_order') . '" onClick="return confirm(\'' . NEW_ORDER . '\')">' . BUTTON_NEW_ORDER . '</a></td></tr></table>');

				$contents[] = array (
					'text' => '<br />' . TEXT_DATE_ACCOUNT_CREATED . ' ' . xtc_date_short($cInfo->date_account_created
				));
				$contents[] = array (
					'text' => '<br />' . TEXT_DATE_ACCOUNT_LAST_MODIFIED . ' ' . xtc_date_short($cInfo->date_account_last_modified
				));
				$contents[] = array (
					'text' => '<br />' . TEXT_INFO_DATE_LAST_LOGON . ' ' . xtc_date_short($cInfo->date_last_logon
				));
				$contents[] = array (
