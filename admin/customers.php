<?php
/* --------------------------------------------------------------
   $Id: customers.php,v 1.11 2004/04/14 19:14:06 fanta2k Exp $   
  ...
  if ($_GET['action']) {
    ...
    $stat = xtc_db_fetch_array($stat_query);
          ...
          $sql_data_array = array('customers_id' => xtc_db_prepare_input($customers['customers_id']),
                                  'customers_cid' => '',
                                  'customers_vat_id' => xtc_db_prepare_input($stat['customers_vat_id']),
                                  'customers_status' => xtc_db_prepare_input($stat['customers_status']),
...
<?php
  if ($_GET['action'] == 'edit' || $_GET['action'] == 'update') {
    $customers_query = xtc_db_query("select c.customers_gender, c.customers_vat_id, c.customers_status, c.member_flag, c.customers_firstname,c.customers_cid, c.customers_lastname, c.customers_dob, c.customers_email_address, a.entry_company, a.entry_street_address, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_default_address_id = a.address_book_id where a.customers_id = c.customers_id and c.customers_id = '" . $_GET['cID'] . "'");
