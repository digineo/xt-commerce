<?php
/* --------------------------------------------------------------
   $Id: new_categorie.php,v 1.6 2004/03/25 13:41:36 fanta2k Exp $
...
echo '<td><span class="main">'.xtc_draw_pull_down_menu('categorie_template',$files,$default_value);
?>
        </span></td>
      </tr>
      <tr>
...
if (strstr($category['group_ids'],'c_'.$customers_statuses_array[$i]['id'].'_group')) {
...
<?php } ?>
      ...
      <tr>
        <td class="main" align="right"><?php echo xtc_draw_hidden_field('categories_date_added', (($cInfo->date_added) ? $cInfo->date_added : date('Y-m-d'))) . xtc_draw_hidden_field('parent_id', $cInfo->parent_id) . xtc_image_submit('button_save.gif', IMAGE_SAVE,'style="cursor:hand" onClick="return confirm(\''.SAVE_ENTRY.'\')"') . '&nbsp;&nbsp;<a href="' . xtc_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $_GET['cID']) . '">' . xtc_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
      </form></tr>