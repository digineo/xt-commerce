<?php
/* --------------------------------------------------------------
   $Id: orders.php,v 1.17 2004/06/05 10:53:44 fanta2k Exp $
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
 ...
 <!-- Bestellbearbeitung Anfang -->
   <a href="<?php echo xtc_href_link(FILENAME_ORDERS_EDIT, 'oID='.$_GET['oID'].'&cID=' . $order->customer['ID']);?>"><img src="<?php echo DIR_WS_CATALOG.'lang/'.$_SESSION['language'].'/admin/images/buttons/'; ?>button_edit.gif" border="0"></a>
...
echo '<a href="' . xtc_href_link(FILENAME_GV_MAIL, xtc_get_all_get_params(array('cID', 'action')) . 'cID=' . $order->customer['ID']) . '">' . xtc_image_button('button_gift.gif', IMAGE_ACCOUNTING) . '</a>';
}
?>

   <img src="<?php echo DIR_WS_CATALOG.'lang/'.$_SESSION['language'].'/admin/images/buttons/'; ?>button_invoice.gif" style="cursor:hand" onClick="window.open('<?php echo xtc_href_link(FILENAME_PRINT_ORDER,'oID='.$_GET['oID']); ?>', 'popup', 'toolbar=0, width=640, height=600')">
   <img src="<?php echo DIR_WS_CATALOG.'lang/'.$_SESSION['language'].'/admin/images/buttons/'; ?>button_packingslip.gif" style="cursor:hand" onClick="window.open('<?php echo xtc_href_link(FILENAME_PRINT_PACKINGSLIP,'oID='.$_GET['oID']); ?>', 'popup', 'toolbar=0, width=640, height=600')">
 <?php   
 
// BMC Delete CC Info Start
 echo '<a href="' . xtc_href_link(FILENAME_ORDERS, 'oID=' . $_GET['oID'] . '&action=deleteccinfo') . '">' . xtc_image_button('button_removeccinfo.gif', RemoveCVV) . '</a>&nbsp;';
// BMC Delete CC Info END
 echo '<a href="' . xtc_href_link(FILENAME_ORDERS, 'page=' . $_GET['page'] . 'oID=' . $_GET['oID']) . '">' . xtc_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
      </tr>
<?php
