<?php
/* --------------------------------------------------------------
   $Id: new_attributes_functions.php,v 1.3 2004/05/31 10:15:35 fanta2k Exp $   
  ...
  function checkAttribute($current_value_id, $current_pid, $current_product_option_id) {
    ...
    if ($isFound) {
      while($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
        ...
        $dl_sql = mysql_query("SELECT products_attributes_maxdays, products_attributes_filename, products_attributes_maxcount FROM products_attributes_download WHERE products_attributes_id = '" . $line['products_attributes_id'] . "'") or die(mysql_error());
        $dl_res = mysql_fetch_array($dl_sql);
