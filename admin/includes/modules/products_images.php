<?php
/* --------------------------------------------------------------
   $Id: products_images.php,v 1.2 2004/06/11 18:49:39 fanta2k Exp $
   ...
   if ($_GET['action']=='new_product') {
        ...
        echo '<tr><td colspan="4">'. xtc_draw_separator('pixel_trans.gif', '1', '10').'</td></tr>';        
        if ($mo_images[$i]["image_name"]){ echo '<tr><td colspan="4"><table><tr><td align="center" class="main" width="'.(PRODUCT_IMAGE_THUMBNAIL_WIDTH+15).'">' . xtc_image(DIR_WS_CATALOG_THUMBNAIL_IMAGES.$mo_images[$i]["image_name"], 'Image '.($i+1)) .'</td>';} else { echo '<tr>'; }
        ...
        if (isset($mo_images[$i]["image_name"])){ echo '</tr><tr><td align="center" valign="middle">' . xtc_draw_selection_field('del_mo_pic[]', 'checkbox', $mo_images[$i]["image_name"]) .' '. TEXT_DELETE . '</td></tr></table>'; } else { echo '</td></tr>';}      
        }
        }        
