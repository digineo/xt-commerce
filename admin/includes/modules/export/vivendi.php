<?php
/* -----------------------------------------------------------------------------------------
   $Id: vivendi.php,v 1.1 2004/04/26 20:19:52 fanta2k Exp $
...
define('MODULE_VIVENDI_FILE_TITLE' , '<hr noshade>Dateiname');
...
// include needed functions
  ...
  class vivendi {
    ...
    function vivendi() {
      ...
      $this->enabled = ((MODULE_VIVENDI_STATUS == 'True') ? true : false);
    ...
    function process($file) {
        ...
        $schema = 'Artikelnr;Produktname;Preis;Link;Beschreibung;Lieferstatus;Kategorie;Hersteller;Bild' . "\n";
    ...
    return array('text' =>  EXPORT_STATUS_TYPE.'<br>'.
                            ...
                            '<a href="' . xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=vivendi') . '">' .
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_VIVENDI_STATUS'");
    ...
    function install() {
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_VIVENDI_FILE', 'vivendi.csv',  '6', '1', '', now())");
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_VIVENDI_STATUS', 'True',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
}
