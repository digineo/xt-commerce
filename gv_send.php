<?php
/* -----------------------------------------------------------------------------------------
   $Id: gv_send.php,v 1.2 2004/02/22 16:15:30 fanta2k Exp $
...
// if the customer is not logged on, redirect them to the login page
  ...
  if ($_GET['action'] == 'process') {
    ...
    if ($new_amount<0) {
      ...
      $gv_query=xtc_db_query("insert into " . TABLE_COUPONS . " (coupon_type, coupon_code, date_created, coupon_amount) values ('G', '" . $id1 . "', NOW(), '" . $_POST['amount'] . "')");
      $insert_id = xtc_db_insert_id($gv_query);
      $gv_query=xtc_db_query("insert into " . TABLE_COUPON_EMAIL_TRACK . " (coupon_id, customer_id_sent, sent_firstname, sent_lastname, emailed_to, date_sent) values ('" . $insert_id . "' ,'" . $_SESSION['customer_id'] . "', '" . addslashes($gv_customer['customers_firstname']) . "', '" . addslashes($gv_customer['customers_lastname']) . "', '" . $_POST['email'] . "', now())");
      ...
      $smarty->assign('AMMOUNT',$xtPrice->xtcFormat($_POST['amount'],true));
  ...
  if ($_GET['action'] == 'send' && !$error) {
    ...
    // validate entries
      ...
      $smarty->assign('MAIN_MESSAGE', sprintf(MAIN_MESSAGE, $xtPrice->xtcFormat($_POST['amount'],true), stripslashes($_POST['to_name']), $_POST['email'], stripslashes($_POST['to_name']), $xtPrice->xtcFormat($_POST['amount'],true), $send_name));
      if ($_POST['message']) {
