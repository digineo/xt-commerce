...
require_once(DIR_FS_INC . 'xtc_count_products_in_category.inc.php');
  ...
  function get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false, $cPath = '') {
    ...
    while ($categories = xtc_db_fetch_array(&$categories_query,true)) {
      if ($exclude != $categories['categories_id']) $category_tree_array[] = array('id'    => $categories['categories_id'],
		 	 					 																												 	 			 'text'  => $spacing . $categories['categories_name'],																																								 
																																								 'link'  => DIR_WS_CATALOG . 'index.php?cPath=' . $cPath . $categories['categories_id']
																																								 );
  ...
  while ($categories = xtc_db_fetch_array(&$categories_query,true)) {
    $module_content[]=array(
                            ...
                            'CAT_LINK'  => DIR_WS_CATALOG . 'index.php?cPath=' . $categories['categories_id'],
