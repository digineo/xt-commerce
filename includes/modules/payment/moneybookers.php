<?php
/* -----------------------------------------------------------------------------------------
   $Id: moneybookers.php,v 1.3 2004/03/01 00:16:31 fanta2k Exp $
...
// class methods
    ...
    function process_button() {
      ...
      $process_button_string = xtc_draw_hidden_field('pay_to_email', MODULE_PAYMENT_MONEYBOOKERS_EMAILID) .
                               xtc_draw_hidden_field('transaction_id', $this->transaction_id) .
                               xtc_draw_hidden_field('return_url', xtc_href_link(FILENAME_CHECKOUT_PROCESS, 'trid='.$this->transaction_id, 'NONSSL', false)) .
                               xtc_draw_hidden_field('cancel_url', xtc_href_link(FILENAME_CHECKOUT_PAYMENT, MODULE_PAYMENT_MONEYBOOKERS_ERRORTEXT1.$this->code.MODULE_PAYMENT_MONEYBOOKERS_ERRORTEXT2, 'SSL', true, false)) .
                               xtc_draw_hidden_field('status_url', 'mailto:' . MODULE_PAYMENT_MONEYBOOKERS_EMAILID) .
                               xtc_draw_hidden_field('language', $mbLanguage) .
                               xtc_draw_hidden_field('pay_from_email', $order->customer['email_address']) .
                               xtc_draw_hidden_field('amount', $amount) .
                               xtc_draw_hidden_field('currency', $mbCurrency) .
                               xtc_draw_hidden_field('detail1_description', STORE_NAME) .
                               xtc_draw_hidden_field('detail1_text', MODULE_PAYMENT_MONEYBOOKERS_ORDER_TEXT . strftime(DATE_FORMAT_LONG)) .
                               xtc_draw_hidden_field('firstname', $order->billing['firstname']) .
                               xtc_draw_hidden_field('lastname', $order->billing['lastname']) .
                               xtc_draw_hidden_field('address', $order->billing['street_address']) .
                               xtc_draw_hidden_field('postal_code', $order->billing['postcode']) .
                               xtc_draw_hidden_field('city', $order->billing['city']) .
                               xtc_draw_hidden_field('state', $order->billing['state']) .
                               xtc_draw_hidden_field('country', $mbCountry) .
                               xtc_draw_hidden_field('confirmation_note', MODULE_PAYMENT_MONEYBOOKERS_CONFIRMATION_TEXT);
