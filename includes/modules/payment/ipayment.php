...
/* -----------------------------------------------------------------------------------------
   $Id: ipayment.php 187 2007-02-24 14:58:53Z mzanier $   
...
class ipayment {
		...
		$selection = array (
			...
			'fields' => array (
				array (
					...
					'field' => xtc_draw_input_field('ipayment_cc_owner',
			...
			'field' => xtc_draw_pull_down_menu('ipayment_cc_expires_month',
			$expires_month
		) . '&nbsp;' . xtc_draw_pull_down_menu('ipayment_cc_expires_year', $expires_year)), array (
			'title' => MODULE_PAYMENT_IPAYMENT_TEXT_CREDIT_CARD_CHECKNUMBER,
			'field' => xtc_draw_input_field('ipayment_cc_checkcode',
			'',
			'size="4" maxlength="3"'
		) . '&nbsp;<small>' . MODULE_PAYMENT_IPAYMENT_TEXT_CREDIT_CARD_CHECKNUMBER_LOCATION . '</small>')));
	...
	function confirmation() {
		...
		$confirmation = array (
			...
			'fields' => array (
				...
				array (
					'title' => MODULE_PAYMENT_IPAYMENT_TEXT_CREDIT_CARD_NUMBER,
					'field' => substr($_POST['ipayment_cc_number'],
					0,
					4
				) . str_repeat('X',
				 (strlen($_POST['ipayment_cc_number']
			) - 8
		)) . substr($_POST['ipayment_cc_number'], -4)), array (
			'title' => MODULE_PAYMENT_IPAYMENT_TEXT_CREDIT_CARD_EXPIRES,
			'field' => strftime('%B, %Y',
			mktime(0,
			0,
			...
			1,
			'20' . $_POST['ipayment_cc_expires_year']
		)))));
	...
	function after_process() {
		...
		if ($this->order_status)
			xtc_db_query("UPDATE " .
			TABLE_ORDERS . " SET orders_status='" . $this->order_status . "' WHERE orders_id='" . $insert_id . "'");
	...
	function keys() {
		return array (
			'MODULE_PAYMENT_IPAYMENT_STATUS',
			'MODULE_PAYMENT_IPAYMENT_ALLOWED',
