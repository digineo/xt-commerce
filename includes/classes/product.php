...
class product {
	...
	function getExpireDate() {
		...
		$products_expires_query = xtDBquery("select expires_date from " . TABLE_SPECIALS . "  where products_id='" . $this->pID . "' and status = 1");
		$products_expires = xtc_db_fetch_array($products_expires_query, true);
		return $products_expires['expires_date'];
	...
	function getReviews() {
		...
		if (xtc_db_num_rows($reviews_query, true)) {
			...
			while ($reviews = xtc_db_fetch_array($reviews_query, true)) {
				...
				), 'RATING' => xtc_image('templates/' . CURRENT_TEMPLATE . '/img/stars_' . $reviews['reviews_rating'] . '.gif', sprintf(TEXT_OF_5_STARS, $reviews['reviews_rating'])), 'TEXT' => $reviews['reviews_text']);
	...
	function getCrossSells() {
		...
		if (xtc_db_num_rows($cs_groups, true) > 0) {
			while ($cross_sells = xtc_db_fetch_array($cs_groups, true)) {
				...
				$cross_query = "select distinct p.products_id,p.products_fsk18,
																										p.products_tax_class_id,
																										p.products_image,
																										pd.products_name,
				                               	    													p.products_quantity,
	...
	function getReverseCrossSells() {
		...
		$cross_query = xtDBquery("select distinct p.products_id,p.products_fsk18,
																								 p.products_tax_class_id,
																		                                                 p.products_image,
																		                                                 pd.products_name,
                               	   																		 p.products_quantity,
	...
	function getGraduated() {
		...
		for ($i = 0, $n = sizeof($staffel); $i < $n; $i++) {
			...
			$staffel_data[$i] = array (
				...
				true,
				$this->data['products_tax_class_id']
			));
		}
	...
	function getBuyNowButton($id, $name) {
		global $PHP_SELF;
		return '<a href="' . xtc_href_link(basename($PHP_SELF), 'action=buy_now&BUYproducts_id=' . $id . '&' . xtc_get_all_get_params(array (
			'action'
	...
	function buildDataArray(& $array, $image = 'thumbnail') {
		...
		return array (
			...
			'PRODUCTS_VPE' => $this->getVPEtext($array,
