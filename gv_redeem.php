<?php
/* -----------------------------------------------------------------------------------------
   $Id: gv_redeem.php,v 1.1 2004/02/17 21:13:26 fanta2k Exp $
...
// check for a voucher number in the url
  if (isset($_GET['gv_no'])) {
    $error = true;
    $gv_query = xtc_db_query("select c.coupon_id, c.coupon_amount from " . TABLE_COUPONS . " c, " . TABLE_COUPON_EMAIL_TRACK . " et where coupon_code = '" . $_GET['gv_no'] . "' and c.coupon_id = et.coupon_id");
