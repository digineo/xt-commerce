...
if (!$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_manufacturers.html', $cache_id) || !$cache) {
	...
	$manufacturers_query = xtc_db_queryCachedNoCheck($manufacturers_query);
	if (xtc_db_num_rowsNoCheck($manufacturers_query) <= MAX_DISPLAY_MANUFACTURERS_IN_A_LIST) {
		// Display a list
		$manufacturers_list = '';
		while ($manufacturers = xtc_db_fetch_arrayNoCheck($manufacturers_query)) {
